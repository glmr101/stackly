...
export default function EditAccount() {
  ...
  useEffect(() => {
    if (accountToEdit) {
      ...
      if (accountToEdit.cardCategory) {
        setCardCategory(accountToEdit.cardCategory as CardCategory);
      }
      
      if (accountToEdit.cardNetwork) {
        setCardNetwork(accountToEdit.cardNetwork as CardNetwork);
      }
      
      const foundBank = PHILIPPINE_BANKS.find(
        (b) => b.code === accountToEdit.bankCode || b.shortName === accountToEdit.institution
      );
      
      if (foundBank) {
        setSelectedBank(foundBank);
      } else {
        const otherBank = PHILIPPINE_BANKS.find((b) => b.id === "other") || defaultBank;
        setSelectedBank(otherBank);
        setCustomInstitution(accountToEdit.institution);
      }
    }
  }, [accountToEdit, defaultBank]);
  ...
  const handleSave = () => {
    ...
    if (!name.trim() || !institutionName || !id) {
      return;
    }
